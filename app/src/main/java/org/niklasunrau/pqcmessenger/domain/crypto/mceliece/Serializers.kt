...
import org.niklasunrau.pqcmessenger.domain.crypto.toBitArrayLong
...
class ElementSerializer : KSerializer<UnivariatePolynomialZp64> {
    ...
    override fun deserialize(decoder: Decoder): UnivariatePolynomialZp64 {
        val coeffs = decoder.decodeString().toBitArrayLong()
...
class PolySerializer : KSerializer<UnivariatePolynomial<UnivariatePolynomialZp64>> {
    ...
    override fun deserialize(decoder: Decoder): UnivariatePolynomial<UnivariatePolynomialZp64> {
        ...
        val coeffs = coeffsInString.map { UnivariatePolynomialZp64.create(2, it.toBitArrayLong()) }.toTypedArray()
        return UnivariatePolynomial.create(ff2m, *coeffs)
    }
}

@OptIn(ExperimentalSerializationApi::class)
class PermMatrixSerializer : KSerializer<Array<ByteArray>> {
    ...
    override fun deserialize(decoder: Decoder): Array<ByteArray> {
        ...
        return Array(list.size) { row -> ByteArray(list.size) { if (list[row] == it) 1 else 0 } }
    }

}

@OptIn(ExperimentalSerializationApi::class)
class MatrixSerializer : KSerializer<Array<ByteArray>> {
    private val delegateSerializer = serializer<List<String>>()
